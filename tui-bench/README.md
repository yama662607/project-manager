...
プロジェクトのルートディレクトリにある `install.sh` を使用して簡単にインストールできます。
...
このスクリプトは、自動的に依存関係（Rust）をチェックし、ビルドを行い、実行ファイルを `~/.local/bin/pm` にインストールします。
...
ビルドされたバイナリ（`target/release/pm`）を任意のPATHが通った場所に配置するか、以下のように一時的にPATHに追加してください：
